...
All partials are consumed with lowercase `@use` and an explicit namespace:
...
When the partials are part of your own source tree the paths become relative,
e.g. `@use "partials/functions" as f;`. The public entry
`src/scss/main.scss` loads `reset`, `variables`, `colors` and `utils`, then
calls:
...
Source: `partials/_functions.scss`.
...
Source: `partials/_variables.scss`.
...
```scss
color: c.get-color("neutral", 500);  // hsl(0, 0%, 35%)
color: c.get("info");                // shade 500
border: 1px solid c.alpha("warning", 400, 0.25);
```
...
Source: `partials/_theme.scss`.

| Signature | Behaviour |
| --- | --- |
| `theme($mode: "dark", $overrides: ())` | Thin hook that delegates to `colors.theme()`. |

```scss
@include t.theme("dark"); // inverts palettes, emits :root[data-theme="dark"]
```
...
Source: `partials/_breakpoints.scss`.
...
@include bp.breakpoint("lg", down) { … }
@include bp.bp("md", up) { … } // the alias forwards to breakpoint("md", up)
```
...
```scss
@include bp.xxl { … }      // @media (min-width: 1536px)
@include bp.xxxl { … }     // @media (min-width: 1920px)
@include bp.x2l-down { … } // @media (max-width: calc(1536px - 0.02px))
```
...
.cards { @include g.grid-autofill(240px, 1fr, 1rem); }

.dashboard {
  @include g.grid-breakpoint-columns(("default": 1, "md": 2, "xl": 4), 1rem);
}
```
...
```scss
main { @include g.grid-placement(2, 4, 1, 2); }  // grid-column: 2/4; grid-row: 1/2
header { @include g.grid-item-full(); }           // grid-column: 1 / -1
```
...
```scss
.stack { @include g.grid-stack(grid, null, ("span", "div"), 2, 1); }
...
```scss
.toolbar { @include fl.flex-container(row, nowrap, 0.5rem, center, space-between); }
.logo    { @include fl.flex-item(0, 0, auto); }
```
...
Source: `partials/_utils.scss` (facade) → `partials/utils/*.scss`. The facade
forwards every piece; `utils/_index.scss` exposes `generate-all-utilities()`.

### Utility token maps (all `!default`) — `utils/_maps.scss`
...
**`$gap-sizes-map`** — `0` 0 · `1` 0.25rem · `2` 0.5rem · `3` 0.75rem ·
`4` 1rem · `5` 1.25rem · `6` 1.5rem · `8` 2rem. Used only by the flex gap
generator.
...
```scss
@include u.utils-classes((auto, hidden), overflow, "overflow");
// .overflow-auto, .overflow-hidden

@include u.utils-classes((auto: auto, hidden: hidden), overflow, "overflow");
// identical output

@include u.utils-classes((flex, grid), display);
// .flex, .grid (no prefix)
```
...
| Generator | Classes |
| --- | --- |
| `sizing.get-sizing-classes($map: maps.$sizing-map)` | `.w-*`, `.min-w-*`, `.max-w-*`, `.h-*`, `.min-h-*`, `.max-h-*`. |
| `flex.generate-flex-utilities($direction: …, $wrap: …, $align: …, $justify: …, $gap: …)` | `.flex-row` / `.flex-col` / reverses, `.flex-wrap` / `.flex-nowrap`, `.items-*`, `.justify-*`, `.gap-0` … `.gap-8`. |
| `effects.generate-effects-utilities()` | `.shadow-*`, `.rounded-*`, `.z-*`, `.duration-*`, `.ease-*`, `.opacity-*`. |
| `layout.generate-layout-utilities()` | display, position, `.overflow-*`, `.text-left/center/right/justify`, `.font-*`, `.whitespace-*`, `.wrap-break-word`. |
| `spacing.generate-spacing-utilities($map: maps.$spacing-map)` | `.p-*`. |
| `utils.generate-all-utilities()` | Sizing + flex + effects + layout. |

### Automatic output when the module is loaded

Loading `utils` (or `utils/index`) executes `_spacing.scss` and
`_effects.scss` top-level rules. The compiled sheet therefore always contains:

```css
.p-0, .p-05, .p-base, .p-1, .p-1-5, .p-2-5, .p-3-5, .p-4, …, .p-96 {
  padding: <value>;
}
.gap { gap: 1rem; }
...
## The apply system (`as a`)

Source: `partials/_apply.scss`.
...
@include a.register-utility("fade-in", (animation: fade-in 300ms ease));
.card {
...
`_apply.scss` registers a large set of utilities when loaded. The registry is
derived from the **same maps** that drive the class generators, so the names
match the map keys exactly (`p-05`, `m-2-5`, `gap-base`, `rounded-lg`,
...
- **Spacing** — for every key of `$spacing-map`: `p-*`, `px-*`, `py-*`,
  `pt-*`, `pb-*`, `pl-*`, `pr-*`, `m-*`, `mx-*`, `my-*`, `mt-*`, `mb-*`,
  `ml-*`, `mr-*`, `gap-*`, `gap-x-*`, `gap-y-*`.
...
class. Names that are only in the registry (margin and directional padding
classes, `text-xs`…`text-4xl`, `flex-1`, `grid-cols-*`, `col-span-full`,
`.border`, `transition*`, `gap-x-*`/`gap-y-*`, …) must be used through
`apply()` and never assumed to exist as classes in the compiled CSS.
