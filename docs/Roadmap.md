...
- [x] Pure functions — `rem()`, `px()`, `to-unit()`, `strip-unit()`,
      ...
      `to-class()`.
- [x] Utility engine driven by `utils/_maps.scss` — spacing classes active by
      default, sizing/flex/effects/layout generators opt-in.
- [x] `@apply`-style registry (`register-utility` + `apply`) kept in sync with
      the utility maps.
- [x] Modern CSS reset whose defaults are overrideable custom properties.
- [x] Vite demo (`index.html` + `demo/main.js`) with HMR and a PurgeCSS
      production build into `demo-dist/`.
- [x] Example components built on `@apply` (`components/_index.scss`).
- [x] Test suite (`node --test`, 26 tests over fixtures) and full English
      documentation (README + `docs/` + CONTRIBUTING/SECURITY/CHANGELOG).

## Planned

### Utilities as real classes

- [ ] Generate **margin utilities as CSS classes** (`.m-*`, `.mx-*`, `.mt-*`,
      …) and directional padding classes. Today they exist only in the
      `@apply` registry, not in the compiled sheet.
- [ ] Decide and implement a consistent story for gap/`gap-x`/`gap-y`
      utilities beyond the fixed 0–8 step set of `$gap-sizes-map`.

### Token architecture

- [ ] **Unify the two spacing scales**: `$spacing-scale` (token variables) and
      `$spacing-map` (utility classes/registry) currently hold different key
      sets. Unifying them would make `--spacing-*` variables and `.p-*` classes
      one source of truth.
