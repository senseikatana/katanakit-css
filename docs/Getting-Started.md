...
| `katanakit-css/src/scss/partials/*`     | Individual modules you compose into a custom sheet.                   |
...
as the precompiled sheet: reset, token custom properties in `:root`, color
custom properties and utilities, and all utility generators.
...
.card {
  ...
  @include bp.md {
    @include a.apply(flex-row, gap-6);
  }
}
```
...
Remember: just loading the `utils` module emits `.p-*`, `.gap`, `.shadow` and
`.rounded` automatically. `generate-all-utilities()` adds sizing, flex,
effects and layout classes. Skip the generators you do not need.
...
Utility maps (`partials/utils/_maps.scss`) are configured the same way if you
...
@include c.generate-css-vars();
@include t.theme("dark"); // palettes inverted under :root[data-theme="dark"]
```
...
Functions give you the raw colors at build time: `c.get-color("info", 300)`,
`c.get("success")`, `c.alpha("danger", 500, 0.4)`.
