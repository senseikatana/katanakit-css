...
Utilities for controlling an element's margin. Every value of `$spacing-map`
is emitted for the seven prefixes: `m-*` (all sides), `mx-*`/`my-*`
(horizontal/vertical) and `mt-*`/`mr-*`/`mb-*`/`ml-*` (one side).
...
Control the horizontal margin with `mx-*` and the vertical margin with
`my-*`:
